 ...
 *   1. Duplique um bloco existente em LOCALE_DATA abaixo
 *   2. Troque o código (ex: 'es-ES') e traduza apenas os valores
 *   3. Mantenha os placeholders {0}, {1}, {2}... intocados
 *   4. Adicione o locale no array AVAILABLE_LOCALES
 *   5. Adicione <option value="es-ES">ES</option> no #langSwitcher do HTML
 *
...
const I18N = (() => {
  ...
  "sidebar.size.label": "TAMANHO",
  "sidebar.size.byte": "BYTE",
  "sidebar.size.byte.sub": "8 bit",
  "sidebar.size.word": "WORD",
  "sidebar.size.word.sub": "16 bit",
  "sidebar.size.dword": "DWORD",
  "sidebar.size.dword.sub": "32 bit",
  "sidebar.size.qword": "QWORD",
  "sidebar.size.qword.sub": "64 bit",
  "sidebar.reg.label": "REGISTRADOR",
  ...
  "help.ops": "<h3>OPERATIONS</h3><p><span class=\"hl\">STORE</span> — Writes the register value to memory, byte by byte. The nibble being sent is highlighted and an animated packet flies to the memory cell.</p><p><span class=\"hl-b\">LOAD</span> — Reads bytes from memory into the register. The register starts at zero and is filled byte by byte in real time — you see the value being built!</p><p><span style=\"color:var(--amb)\">STEP</span> — Executes the full <strong>FETCH → DECODE → EXECUTE</strong> cycle, updating PC, registers, memory and stack per the instruction.</p><p><span style=\"color:var(--red)\">CLEAR</span> — Resets memory and registers.</p><div class=\"info-box\">💡 Click any memory cell to select that address.<br>💡 Use DWORD to see the full Little vs Big Endian effect.<br>💡 Slow down to follow each byte carefully.<br>💡 Save 💾 and load 📂 simulations to share.</div>",
  ...
  // ── API pública ─────────────────────────────────────────────────────────────
  return { t, init, load, setLocale, applyDOM,
